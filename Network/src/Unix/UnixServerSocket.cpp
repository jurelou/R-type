...
UnixServerSocket::UnixServerSocket(Server *s)
{
...
bool	UnixServerSocket::init(const std::string& host, const std::string& _port)
{
  int	i;
  std::stringstream ss;
  ss << _port;
...
void	UnixServerSocket::addNewPeer(void * peer)
{
    static int id = 0;
...
TcpUdpPair     UnixServerSocket::getUserPair(int sock) const
{
    if (sock) {
...
void        UnixServerSocket::callBack(std::list<int>::iterator& it)
{
...
ISocket	                *UnixServerSocket::Accept(void *sockType)
{
  int			        acceptSock;
...
std::map<int, ISocket *>   UnixServerSocket::getClientsSocketMap() const
{ return _clientsSocketMap; }

std::map<std::string, ISocket *>   UnixServerSocket::getClientsSocketUdpMap() const
